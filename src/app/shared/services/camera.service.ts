  ...
  */
  async takePictureAndGetData(): Promise<{ imagePath: string, file: File, latitude: number | null, longitude: number | null, date: string, time: string }> {
    ...
    try {
      ...
      return { imagePath: imagePath!, file: file!, latitude, longitude, date, time };
    } catch (error) {
      console.error('Error obteniendo la geolocalización', error);
      const now = new Date();
      return { imagePath: imagePath!, file: file!, latitude: null, longitude: null, date: now.toLocaleDateString(), time: now.toLocaleTimeString() };
    }
  }
